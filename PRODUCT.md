...
PocketCFO primarily serves people managing everyday spending from one payday to the next. They need a trustworthy answer to a practical question: how much can I safely spend today after the money already committed to bills, debt, savings, and other obligations is protected?
...
PocketCFO turns current cash and upcoming commitments into one reliable safe-to-spend number for today. Success means the user can make a daily spending decision without rebuilding a monthly budget in their head or accidentally spending money that already has a job.
...
PocketCFO is a forward-looking pay-cycle planner. Its defining mechanism is the Horizon engine, which starts with liquid cash, protects committed obligations and savings, accounts for the days until payday, applies any voluntary spend limit, and then shapes availability through pacing. Most budgeting tools explain where money went; PocketCFO tells the user what is safely available now.

The dashboard number is the product promise. Every screen that presents or changes daily availability must agree with the same calculation and clearly explain any spend limit or pacing effect.

## Operating Context

Users typically check PocketCFO on a phone before or after a purchase, during a daily review, and when income, bills, or payday timing changes. The main loop is:

1. Enter current cash, payday, recurring bills, and relevant goals.
2. Check the safe-to-spend amount for today.
...
- The Horizon calculation in `src/core/math.ts` is the source of truth for safe-to-spend values.
...
The central promise should remain easy to repeat: one reliable number for what is safe to spend today.
...
3. **Make the next action obvious.** Prioritize what the user can safely do today over retrospective complexity.
